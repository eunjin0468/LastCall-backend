...
public class AuctionEventConsumer {
  ...
  private void sendToDLQ(AuctionEvent event, String queueType) {
    try {
      String dlqRoutingKey = AuctionRabbitMqConfig.DLQ_ROUTING_KEY;
      rabbitTemplate.convertAndSend(AuctionRabbitMqConfig.DLX_NAME, dlqRoutingKey, event);
      log.info("[RabbitMQ] DLQ로 메시지 전송 완료: queueType={}, auctionId={}", queueType, event.getAuctionId());
    } catch (Exception e) {
      log.error("[RabbitMQ] DLQ 전송 실패: queueType={}, auctionId={}", queueType, event.getAuctionId(), e);
    }
  }
